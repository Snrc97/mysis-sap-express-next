...
exports.getById = async (req, res) => {
  try {
    ...
    if (item) {
      ...
      res.status(404).customJson({ success: false, msg: 'Currency not found' });
    }
  } catch (err) {
    res.status(500).customJson({ success: false, msg: err.message });
  }
...
exports.create = async (req, res) => {
  try {
    const newItem = await Currency.create(req.body);
...
exports.delete = async (req, res) => {
  try {
    ...
    if (item) {
      await item.destroy();
      res.customJson({ msg: 'Currency deleted successfully' });
    } else {
      res.status(404).customJson({ success: false, msg: 'Currency not found' });
    }
  } catch (err) {

...
const Header: React.FC<HeaderProps> = ({ description, headerButtons = [] }) => {
  const [hamburgerMenuOpen, setHamburgerMenuOpen] =
  ...
  const searchInputComponent = () => {
    ...
    return (
      <motion.div
        className="w-57 h-auto absolute mt-26 max-[1390px]:mt-20 min-[1390px]:ml-[150.5px] min-[1390px]:z-[-1]"
      ...
      >
        <form
          className="flex flex-row items-center rounded-xl dark:bg-gray-800 border-2 border-gray-700"
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <input
            ...
            type="text"
            name="search"
            className="w-full h-full outline-none text-black dark:text-white px-4"
          ...
          <Button
            type="submit"
            variant={'secondary'}
            className="bg-green-700 cursor-pointer text-white"
            size={'icon'}
          >
  ...
  return (
    <header className="flex flex-col">
      <div className="z-999 fixed top-0 h-16 flex flex-row w-full h-15 items-center justify-between border-b border-gray-500">
        ...
        <div
          className="w-20 h-full flex flex-row justify-end mx-5 absolute cursor-pointer"
          onClick={() => {
            redirect('/');
          }}
        >
            ...
            <ul className="min-[1390px]:flex gap-4 min-[1390px]:flex-row max-[1390px]:flex-column max-[1390px]:mx-auto max-[1390px]:w-50 flex-wrap w-full h-full justify-end px-3 py-3 items-center">
              <li>
                {isClient && window?.location?.pathname !== '/' && (
                  <Button
                    variant={'default'}
                    className="h-full flex items-center justify-center cursor-pointer bg-green-500 hover:bg-green-600"
                    onClick={() => router.back()}
                  >
                    <Icon
                      name="ArrowLeft"
                      size={40}
                      className="bg-green-700 cursor-pointer rounded bg-transparent"
                    />
                  </Button>
                )}
              </li>
              ...
              {headerButtons?.map((button, index) => (
                <li
                  className="text-nowrap h-full flex flex-row items-center justify-center"
                  key={index}
                >
                  {!button.icon ? (
                    button.link && (
                    ...
                    >
                      ...
                      {button.badge && (
                        <div className="bg-red-500 text-white rounded-full text-[12px] font-bold w-7 h-7 flex items-center justify-center absolute ml-9 mb-8 ">
                          {button.badge}
                        </div>
                      )}
                    </Button>
